...
void evo::window_t::paint( ) {
	...
	*/
	...
	evo::_render->add_text( this->pos->x + 7, this->pos->y + 5, _container->window_accent, 1, this->window_name.c_str( ), 0 );
	const std::string& footer_main = theme::footer_main;
	const std::string& footer_suffix = theme::footer_suffix;
	evo::_render->add_text( this->pos->x + 7,
							this->pos->y + this->size.y - 25,
							_container->window_accent,
							1,
							footer_main.c_str( ),
							0 );
	float footer_offset = _render->text_size( footer_main.c_str( ), 1 ).x;
	if ( !footer_suffix.empty( ) ) {
		evo::_render->add_text( this->pos->x + 7 + footer_offset,
								this->pos->y + this->size.y - 25,
								_container->window_text.modify_alpha( 150 * _container->anim_controler ),
								1,
								footer_suffix.c_str( ),
								0 );
	}
