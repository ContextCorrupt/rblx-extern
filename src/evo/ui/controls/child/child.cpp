...
void evo::child_t::obj( keybind_t* keybind ) {
	if ( get_visible( ) ) { /* draw only if the visible flag is on */
		...
		float row_base_y = this->base_element.y[ this->child_id ];
		float min_row_y = this->base_window.y + 30.0f;
		if (row_base_y < min_row_y)
		{
			row_base_y = min_row_y;
			this->base_element.y[ this->child_id ] = row_base_y;
		}

		/* position keybind widget */
		keybind->base_window = {
			this->base_element.x + ( this->base_size.x - text_s.x ) - 55, row_base_y - 28
		};
		...
		const float consumed_height = 35.0f;
		this->base_element.y[ this->child_id ] = row_base_y + consumed_height;
